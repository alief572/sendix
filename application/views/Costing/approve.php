...
<script>
  ...
  $(document).on('click', '.reject', function(e){
		...
		$("#view2").load(base_url +'index.php/'+ active_controller+'/dialog_reject/'+$(this).data('project_code'));
		$("#ModalView2").modal();
	});
	...
	function DataTables(){
		var dataTable = $('#my-grid').DataTable({
			...
			"ajax":{
				url : base_url +'index.php/'+active_controller+'/data_side_approve',
				type: "post",
				data: function(d){
	...
	$(document).on('click', '.approve', function(){
		...
		function(isConfirm) {
			if (isConfirm) {
				loading_spinner();
				$.ajax({
					url			: base_url+'index.php/'+active_controller+'/approve/'+project_code,
					type		: "POST",
					cache		: false,
					dataType	: 'json',
					processData	: false,
