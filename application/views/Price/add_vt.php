...
$ArrSatuan[0] = 'Select Vehicle Tool';
	...
	<div class="box box-primary">
		...
		<?php
			...
			$tanda 			     = (!empty($header[0]->id))?'edit':'';
	  		...
	  		$rate2 	         = (!empty($header[0]->rate))?number_format($header[0]->rate):'';
		...
		<div class="box-body">
			<div class='form-group row'>
				<label class='label-control col-sm-2'><b>Vehicle Tool <span class='text-red'>*</span></b></label>
				<div class='col-sm-4'>
          <?php
					if(empty($kode)){
        ...
        <label class='label-control col-sm-2'><b>Spesification/Sertification</b></label>
			...
			<div class='form-group row'>
				...
				<div class='col-sm-4'>
					<?php
					 echo form_input(array('id'=>'unit_material','name'=>'unit_material','class'=>'form-control input-md','readonly'=>'readonly','placeholder'=>'Unit'),$unit_material);
					?>
				</div>
          ...
          ?>
			...
			<div class='form-group row'>
				...
				<div class='col-sm-4'>
					<?php
					 echo form_input(array('id'=>'rate','name'=>'rate','class'=>'form-control input-md','placeholder'=>'Price Reference','data-decimal'=>'.','data-thousand'=>'','data-precision'=>'0','data-allow-zero'=>''),$rate2);
					?>
				</div>
			</div>
		</div>
			...
			swal({
				...
				function(isConfirm) {
				  if (isConfirm) {
						...
						var baseurl		= base_url + active_controller +'/add_vt';
						$.ajax({
							url			: baseurl,
							type		: "POST",
							data		: formData,
							...
							success		: function(data){
								if(data.status == 1){
									...
									window.location.href = base_url + active_controller+'/index_vt';
								}
