...
foreach($consumable AS $val => $valx){
	$ArrSatuan[$valx['code_group']] = strtoupper($valx['category'])." - ".strtoupper($valx['spec']);
}
$ArrSatuan[0] = 'Select Man Power';
...
<form action="#" method="POST" id="form_man_power" autocomplete="off">
	<div class="box box-primary">
		...
		<?php
			...
			$rate_ot 	     = (!empty($header[0]->rate_ot))?$header[0]->rate_ot:'';
			$rate_us 	     = (!empty($header[0]->rate_us))?$header[0]->rate_us:'';
			$rate_um 	     = (!empty($header[0]->rate_um))?$header[0]->rate_um:'';
			$brand 	         = (!empty($header[0]->note))?strtoupper($header[0]->note):'';
			...
			$code_groupx     = $category." - ".$spec;
			$kode						 = $this->uri->segment(3);

      $ArrUnit = array();
      foreach($unit AS $val => $valx){
      	$ArrUnit[$valx['category_list']] = strtoupper($valx['category_list']);
        $ArrUnit[$unit_material] = strtoupper($unit_material);
      }
		...
		<div class="box-body">
          ...
          ?>
			...
			<div class='form-group row'>
				...
				<div class='col-sm-4'>
					<?php
					 echo form_input(array('id'=>'brand','name'=>'brand','class'=>'form-control input-md','readonly'=>'readonly','placeholder'=>'Information'),$brand);
					?>
				</div>
			</div>
			<div class='form-group row'>
          ...
          ?>
			...
			<div class='form-group row'>
				...
				<label class='label-control col-sm-2'><b>Price Overtime</b></label>
				<div class='col-sm-4'>
					<?php
					 echo form_input(array('id'=>'rate_ot','name'=>'rate_ot','class'=>'form-control input-md autoNumeric0','placeholder'=>'Price Overtime','data-decimal'=>'.','data-thousand'=>'','data-precision'=>'0','data-allow-zero'=>''),$rate_ot);
					?>
				</div>
			</div>
			<div class='form-group row'>
				<label class='label-control col-sm-2'><b>Price Uang Saku</b></label>
				<div class='col-sm-4'>
					<?php
					 echo form_input(array('id'=>'rate_us','name'=>'rate_us','class'=>'form-control input-md autoNumeric0','placeholder'=>'Price Uang Saku','data-decimal'=>'.','data-thousand'=>'','data-precision'=>'0','data-allow-zero'=>''),$rate_us);
					?>
				</div>
				<label class='label-control col-sm-2'><b>Price Uang Makan</b></label>
				<div class='col-sm-4'>
					<?php
					 echo form_input(array('id'=>'rate_um','name'=>'rate_um','class'=>'form-control input-md autoNumeric0','placeholder'=>'Price Uang Makan','data-decimal'=>'.','data-thousand'=>'','data-precision'=>'0','data-allow-zero'=>''),$rate_um);
					?>
				</div>
			</div>
		</div>
			...
			swal({
				...
				function(isConfirm) {
				  if (isConfirm) {
						...
						var baseurl		= base_url + active_controller +'/add_mp';
						$.ajax({
							url			: baseurl,
							type		: "POST",
							data		: formData,
							...
							success		: function(data){
								if(data.status == 1){
									...
									window.location.href = base_url + active_controller+'/index_mp';
								}
